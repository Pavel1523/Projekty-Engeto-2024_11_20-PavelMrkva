...
if jmeno in user and heslo in password:
    ...
    for attempt in range(3):  # 3 pokusy na zadani spravne vety
        ...
        if veta_i in {"1", "2", "3"}:  # Kontrola, zda je vstup správný
            ...
            for slovo in veta_s:
                ...
                if slovo.isdigit():
                    results_1["suma_cisel"] = results_1["suma_cisel"] + int(
                        slovo
                    )
            print(
                "There are",
            ...
            print(
                "The sum of all the numbers is ", results_1["suma_cisel"], "."
            )
            ...
            print(" ")
            print(36 * "-")
            print(f"{"LEN":>5}|{"OCCURENCES":^24}|{"NR.":<4}")
            ...
            for key, vyskyt in delky_slov_sorted.items():
                print(f"{key:>5}|{('*' * int(vyskyt)):<24}|{vyskyt}")
            print(
                "\nNote:"
                + "\n  LEN.............word length"
    ...
    print(
        "Unregistered user or wrong password.", "\nEnd."
    )  # Pokud spatne jmeno a heslo.
